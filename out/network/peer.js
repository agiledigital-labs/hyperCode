...
class Peer extends EventEmitter {
    ...
    establishDataChannel() {
        let data = this.webrtc.createDataChannel("datachannel", {
            protocol: "tcp",
        });
        ...
        this.webrtc.createOffer((desc) => {
            this.webrtc.setLocalDescription(desc, () => this.send_signal(desc), (e) => console.log("error on setLocalDescription", e));
            ...
            if (signal.type == "offer")
                callback = () => {
                    this.webrtc.createAnswer((answer) => {
                        this.webrtc.setLocalDescription(answer, () => this.send_signal(answer), (e) => console.log("Error setting setLocalDescription", e));
                    }, (e) => console.log("Error creating answer", e));
                };
            this.webrtc.setRemoteDescription(new this.wrtc.RTCSessionDescription(signal), callback, (e) => console.log("Error setRemoteDescription", e));
        }
    ...
    processMessage(msg) {
        var decompressed = Buffer.from(msg.data, "base64");
    ...
    send(message) {
        if (this.self)
            return; // dont send messages to ourselves
        if (!this.connected()) {
