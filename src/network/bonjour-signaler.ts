...
export default class BonjourSignaller extends EventEmitter {
  ...
  searchBonjour() {
    this.browser = bonjour.find({ type: "automesh" }, (service) => {
      let me = this.peergroup.self();
      ...
      if (meta.session < me.id) {
        console.log(
          "peerDiscovery(): peer outranks me - wait for them to offer"
        );
        return;
      }
      this.hearHello(
  ...
  manualHello(host, port, callback) {
    ...
    ws.on("message", (data) => {
      ...
      if (
        greeting.session != me.id &&
        !(greeting.session in this.peergroup.Peers)
      )
  ...
  hearHello(name, session, host, port) {
    ...
    this.peergroup.processSignal(meta, undefined, (offer) =>
      this.sendOffer(host, port, offer)
    );
  }
